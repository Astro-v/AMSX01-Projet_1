...
X = eye(sum(Refneu==3));
X = X(end:-1:1,:);
I = [1 1 1 1 2:(sum(Refneu==1)+1) (sum(Refneu==1)+2):(sum(Refneu==1)+sum(Refneu==2)+1) (sum(Refneu==1)+1):-1:2 (sum(Refneu==1)+sum(Refneu==2)+1):-1:(sum(Refneu==1)+2) (sum(Refneu==1)+sum(Refneu==2)+2):(N0+1+sum(Refneu==1)+sum(Refneu==2))];
J = [1:Nbpt];
PP = sparse(I,J,1);
AA = MM+KK;
AAp = PP*AA*PP';
